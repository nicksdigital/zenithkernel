...
import {
  ...
  ensureCriticalCSS,
  preloadIslandCSS
} from '../../../../src/modules/Rendering/utils/jit-css-loader';
import { initCssSystem } from '../../../../src/modules/Rendering/utils/css-loader';
...
export class ZenithIslandLoader implements IslandLoader {
 ...
 /**
  ...
  private async loadIslandModule(islandName: string): Promise<IslandRegistration | undefined> {
    try {
      // Vite-specific dynamic import using import.meta.glob
      // Adjust the pattern to match your island file extensions (ts, tsx, js)
      const islandModules = import.meta.glob('../../islands/*.{ts,tsx,js}'); // Or more specific like './islands/*Island.{ts,tsx,js}'
      console.log('[ZenithIslandLoader] islandModules found by glob:', islandModules);
      // Construct potential keys based on likely extensions
      const potentialKeys = [
        `../../islands/${islandName}.tsx`,
        `../../islands/${islandName}.ts`,
        `../../islands/${islandName}.js`
      ];

      let actualModulePathKey: string | undefined;
      let moduleImportFunction: (() => Promise<any>) | undefined;

      for (const key of potentialKeys) {
        if (islandModules[key]) {
          actualModulePathKey = key;
          moduleImportFunction = islandModules[key];
          break;
        }
      }

      console.log(`[ZenithIslandLoader] Trying to load island: ${islandName}`);
      console.log(`[ZenithIslandLoader] Constructed potential keys:`, potentialKeys);
      console.log(`[ZenithIslandLoader] Matched key for ${islandName}:`, actualModulePathKey);

      if (moduleImportFunction && actualModulePathKey) {
        const moduleImport = await moduleImportFunction();
        const module = moduleImport as any; // Cast to any or a more specific type

        if (module.default && typeof module.default.mount === 'function') {
          const registration: IslandRegistration = {
            name: islandName,
            component: module.default,
            modulePath: actualModulePathKey, // Store the glob key or relative path
            ...(module.metadata || {})
          };
          this.registerIsland(registration); // Important: Register it so it's found next time
          return registration;
        }
      }   else {
      throw new Error(`Island module "${islandName}" not found in import.meta.glob results. Searched for keys: ${potentialKeys.join(', ')}`);
    }
      throw new Error(`Island module "${islandName}" not found or invalid structure via import.meta.glob.`);
    } catch (error) {
      console.error(`Failed to load island module "${islandName}" using import.meta.glob:`, error);
      return undefined;
    }
  }
