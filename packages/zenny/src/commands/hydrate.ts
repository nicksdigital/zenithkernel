...
// Stub for z object
const z = {
  object: (obj: any) => ({ parse: (data: any) => data }),
  string: () => ({ optional: () => ({ default: (val: any) => val }) }),
  boolean: () => ({ optional: () => ({ default: (val: any) => val }) }),
  record: (type: any) => ({ optional: () => ({}) }),
  infer: null as any
};
...
type HydraManifest = any; // TODO: z.infer<typeof HydraManifestSchema>;
